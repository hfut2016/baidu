			...
			xhr.onreadystatechange = function() {
				if (xhr.readyState == 4) {
					if(xhr.status>=200&&xhr.status<300  ||  xhr.status==304){
						...
						if (rData.filter(item =>  (new RegExp(request.selected)).test(item.question)).length > 0){
							responseData = rData.filter(item =>  (new RegExp(request.selected)).test(item.question))[0];
						}
						...
						myHtml = responseHtml.replace(/id="result_kw">/,`id="result_kw">${request.selected}`);
						myHtml = myHtml.replace(/font-size:small;"><\/div>/,`font-size:larger;">请查看json</div>`);
						sendResponse({result: myHtml, status:'copy'});
					}
				}
			}
